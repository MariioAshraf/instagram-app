...
class StoryRemoteDataSourceImpl implements StoryRemoteDataSource {
  ...
  Future<List<StoryModel>> getMyStories({
    ...
    final List<Future<StoryModel?>> futures =
      ...
      bool hasNewViewers = (localStory.viewersModels == null ||
          localStory.viewersModels!.length != viewersSnapShot.size);
      ...
      if (hasNewViewers) {
        ...
        for (var viewerDoc in viewersSnapShot.docs) {
          ...
          if (!updatedViewers.containsKey(viewerId)) {
            updatedViewersIds[viewerId] = viewedAt;
            final userDoc = await usersCollection.doc(viewerId).get();
            final userModel = UserModel.fromJson(userDoc.data())
                .copyWith(viewStoryAt: viewedAt);
            updatedViewers[viewerId] = userModel;
          }
        }

        return localStory.copyWith(
            viewersIds: updatedViewersIds, viewersModels: updatedViewers);
      }
